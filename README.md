...
during startup. The web UI and API do not accept model uploads. To deploy a new
model, replace the local model artifacts before starting or restarting the
service.
...
`models/preprocessing.joblib`, and `models/model_manifest.json` before startup
when you need to serve a newly trained model.
...
2. Copy the matching notebook artifacts into `models/`.
...
The manifest script validates architecture, feature-column order, threshold, and
artifact hashes. With `VERIFY_MODEL_HASHES=1`, startup fails if the committed
manifest does not match the files in `models/`.
...
Returns the loaded architecture, threshold, device, artifact SHA-256, and model
update mode.
